...
public class HouseKeepingScheduler {
    ...
    private void startDiskCleanup() {
        ...
        sb.append("\n\n\n### ##########################################################################\n");
        sb.append("### Hackathon Disk Cleanup STARTED: ").append(new Date()).append("\n");
        sb.append("### ##########################################################################\n\n");
        log.log(Level.INFO, sb.toString());

        int count = 0;
        File dir = HackathonProperties.getTemporaryDir();
        if (dir.exists()) {
        ...
        log.log(Level.INFO, "### Hackathon HouseKeeping cleaned up {0} temporary files", count);
        try {
            if (count > 10) {
